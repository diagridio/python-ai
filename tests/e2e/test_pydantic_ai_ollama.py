"""E2E tests for PydanticAI agent with Ollama backend via Dapr Workflows."""
...
from tests.e2e.conftest import clear_agent_registration, import_pydantic_ai_agent

_PYDANTIC_WF = "diagrid.agent.pydantic_ai.workflow"


@pytest.mark.ollama
@pytest.mark.integration
def test_pydantic_ai_workflow_lifecycle(ollama_model: str) -> None:
    """Test that a PydanticAI agent completes a full workflow cycle.
    ...
    """
    Agent = import_pydantic_ai_agent()
    ...
    runner = DaprWorkflowAgentRunner(
        agent=agent,
        name="e2e-pydantic-test",
    )
    try:
        runner.start()
        result = runner.run_sync(
            user_message="What is the weather in Tokyo?",
            session_id=f"e2e-pydantic-{uuid.uuid4().hex[:8]}",
            timeout=120,
        )
...
@pytest.mark.ollama
@pytest.mark.integration
def test_pydantic_ai_multi_tool(ollama_model: str) -> None:
    """Test that a PydanticAI agent with 3 tools completes a workflow.

    Validates multi-tool registration works end-to-end, matching
    the simple_agent.py example pattern.
    """
    clear_agent_registration(_PYDANTIC_WF)
    ...
    try:
        ...
        result = runner.run_sync(
            user_message="What is the weather in Tokyo?",
            session_id=f"e2e-pydantic-mt-{uuid.uuid4().hex[:8]}",
            timeout=120,
        )
...
def _tool_step_one(input_data: str) -> str:
...
def _tool_step_two(data: str) -> str:
    """Process data from step one. Second step — fails on first two attempts."""
    with _retry_lock:
...
def _tool_step_three(data: str) -> str:
...
def test_pydantic_ai_retry(ollama_model: str) -> None:
    """Test that a PydanticAI workflow recovers when a tool fails transiently.

    Creates an agent with 3 sequential tools. Tool 2 fails on the first
    2 invocations, then succeeds. The LLM should see the error messages
    ...
    agent = Agent(
        ...
        tools=[_tool_step_one, _tool_step_two, _tool_step_three],
    )

    runner = DaprWorkflowAgentRunner(
        agent=agent,
        name="e2e-pydantic-retry",
        max_iterations=10,
    )
    try:
        runner.start()
        result = runner.run_sync(
            ...
            session_id=f"e2e-pydantic-retry-{uuid.uuid4().hex[:8]}",
            timeout=180,
        )
