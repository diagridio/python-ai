...
CI runs the default extras (``--extra all``), which by design excludes the
``holmesgpt`` extra (it lives in a separate conflict-fork — see the
``[tool.uv].conflicts`` block in ``pyproject.toml``). When ``holmesgpt``
isn't installed, ``holmes`` isn't importable; we skip every test in this
directory instead of failing at conftest-load time.
"""
...
# When ``holmes`` isn't installed in the venv (the default for runs that
# don't pass ``--extra holmesgpt``), skip every test in this directory.
# The rest of the conftest is short-circuited so none of the
# holmes-dependent imports below are evaluated.
if importlib.util.find_spec("holmes") is None:
        ...
        def completion(self, **kwargs):
            ...
            if not self._completions:
                raise AssertionError(
                    "StubLLM.completion called with no scripted responses"
                )
