...
from diagrid.agent.core.metadata.mapping import BaseAgentMapper
...
class TestBaseAgentMapper(unittest.TestCase):
    ...
    def test_extract_provider_openai(self):
        ...
        self.assertEqual(
            BaseAgentMapper._extract_provider("openai.resources"), "openai"
        )
        self.assertEqual(
            BaseAgentMapper._extract_provider("strands.models.openai"), "openai"
        )
    ...
    def test_extract_provider_azure_openai(self):
        ...
        self.assertEqual(
            BaseAgentMapper._extract_provider("azure.openai"), "azure_openai"
        )
        self.assertEqual(
            BaseAgentMapper._extract_provider("AZURE_OPENAI"), "azure_openai"
        )

    def test_extract_provider_anthropic(self):
        """Test provider extraction for Anthropic modules."""
        self.assertEqual(
            BaseAgentMapper._extract_provider("langchain_anthropic.chat_models"),
            "anthropic",
        )
        self.assertEqual(
            BaseAgentMapper._extract_provider("anthropic.client"), "anthropic"
        )
        self.assertEqual(
            BaseAgentMapper._extract_provider("strands.models.anthropic"), "anthropic"
        )

    def test_extract_provider_ollama(self):
        """Test provider extraction for Ollama modules."""
        self.assertEqual(
            BaseAgentMapper._extract_provider("langchain_ollama"), "ollama"
        )
    ...
    def test_extract_provider_google(self):
        ...
        self.assertEqual(
            BaseAgentMapper._extract_provider("langchain_google_genai"), "google"
        )
        self.assertEqual(
            BaseAgentMapper._extract_provider("google.generativeai"), "google"
        )
    ...
    def test_extract_provider_vertexai(self):
        ...
        self.assertEqual(
            BaseAgentMapper._extract_provider("vertexai.client"), "vertexai"
        )

    def test_extract_provider_unknown(self):
        """Test provider extraction for unknown modules."""
        self.assertEqual(
            BaseAgentMapper._extract_provider("some.random.module"), "unknown"
        )
    ...
    def test_extract_provider_priority_azure_over_openai(self):
        ...
        self.assertEqual(
            BaseAgentMapper._extract_provider("azure.openai.client"), "azure_openai"
        )
        self.assertEqual(
            BaseAgentMapper._extract_provider("openai.azure.wrapper"), "azure_openai"
        )
