...
"""
Test script to verify Dapr workflow crash recovery for Deep Agents.

This test:
1. Creates a Deep Agent with custom tools (step_one, step_two, step_three)
2. step_two crashes the process on the first run (after step_one completes)
...
Usage:
    ...
    # First run (will crash during step_two):
    dapr run --app-id deep-agent-crash-test --resources-path ./resources -- python3 test_crash_recovery.py
...
STATE_FILE = Path("/tmp/deepagents_crash_test_state.json")
...
# Create the Deep Agent with all three tools
agent = create_deep_agent(
    model="openai:gpt-4o-mini",
    tools=[step_one, step_two, step_three],
    system_prompt="""You are a sequential task processor that MUST call tools in order.
...
def print_completion(event: dict):
    ...
    log(f"Steps: {event.get('steps', 'N/A')}")
