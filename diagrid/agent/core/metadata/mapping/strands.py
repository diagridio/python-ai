...
class StrandsMapper(BaseAgentMapper):
    ...
    def _extract_from_strands_agent(self, agent: Any) -> Dict[str, Any]:
        ...
        # Basic agent info
        metadata["agent_id"] = getattr(agent, "agent_id", None) or getattr(
            agent, "name", "agent"
        )
        ...
        if system_prompt:
            if isinstance(system_prompt, str):
                ...
                metadata["system_prompt"] = " ".join(
                    str(block) for block in system_prompt
                )
    ...
    def _get_session_agent(
        ...
        try:
            ...
            logger.error(
                f"Failed to read SessionAgent from session {session_id}: {e}",
                exc_info=True,
            )
            return None
    ...
    def _extract_llm_metadata(
        ...
        llm_component = agent_state.get("llm_component") or agent_state.get(
    ...
    def map_agent_metadata(
        ...
        session_agent = (
            self._get_session_agent(session_manager, session_id) if session_id else None
        )
        ...
        if session_agent:
            ...
            instructions = (
                list(instructions_raw) if isinstance(instructions_raw, list) else []
            )
