...
"""
Test script to verify Dapr workflow crash recovery for OpenAI Agents SDK agents.
...
STATE_FILE = Path("/tmp/openai_agents_crash_test_state.json")
...
agent = Agent(
    name="sequential_task_processor",
    instructions="""You are a sequential task processor that MUST call tools in a specific order.
    ...
    try:
        ...
        if not state["workflow_scheduled"]:
            ...
            async for event in runner.run_async(
                ...
                "Call step_one first, then step_two, then step_three.",
...
async def poll_for_completion(runner: DaprWorkflowAgentRunner, workflow_id: str):
    ...
    from diagrid.agent.openai_agents.models import AgentWorkflowOutput
