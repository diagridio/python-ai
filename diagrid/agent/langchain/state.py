...
"""Dapr-backed session store for LangChain agents."""
...
class DaprSessionStore:
    """Session store that persists LangChain conversation state to Dapr state.

    Persists conversation history and session state between workflow
    iterations for LangChain agents.

    Key format: ``langchain-{session_id}-state``
    ...
    def _session_key(self, session_id: str) -> str:
        return f"langchain-{session_id}-state"
