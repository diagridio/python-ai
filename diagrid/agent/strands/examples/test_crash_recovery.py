...
"""
Test script to verify Dapr workflow crash recovery for Strands agents.
...
STATE_FILE = Path("/tmp/strands_crash_test_state.json")
...
def print_completion(event: dict):
    ...
    log(f"{'=' * 60}")
    log(f"Result:\n{event.get('result')}")
    log(f"Tool calls: {len(event.get('tool_calls', []))}")
