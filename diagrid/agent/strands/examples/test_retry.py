...
"""
Test script to verify Dapr workflow activity retry behavior for Strands agents.
...
STATE_FILE = Path("/tmp/strands_retry_test_state.json")
...
agent = Agent(
    ...
    system_prompt="""You are a sequential task processor that MUST call tools in a specific order.
...
def print_completion(event: dict):
    ...
    log(f"{'=' * 60}")
    log(f"Result:\n{event.get('result', event.get('final_response', ''))}")
    print_verification()
