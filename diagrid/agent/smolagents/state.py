...
"""Dapr-backed session store for smolagents agents."""
...
class DaprSessionStore:
    """Session store that persists smolagents conversation state to Dapr state.

    Persists conversation history and session state between workflow
    iterations for smolagents agents.

    Key format: ``smolagents-{session_id}-state``
    ...
    def _session_key(self, session_id: str) -> str:
        return f"smolagents-{session_id}-state"
    ...
    def save_session(
        ...
        Args:
            ...
            data: Session data to persist (messages, etc.).
        """
