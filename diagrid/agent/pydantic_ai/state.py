...
"""Dapr-backed memory store for Pydantic AI agents."""
...
class DaprMemoryStore:
    """Memory store that persists Pydantic AI agent conversation context to Dapr.

    Persists agent conversation context between invocations using
    Dapr state store.

    Key format: ``pydantic-ai-{session_id}-memory``
    ...
    def _memory_key(self, session_id: str) -> str:
        return f"pydantic-ai-{session_id}-memory"
